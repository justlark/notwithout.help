...
**API Challenge Response** is a JSON object with this format:
...
- **Submissions** are encrypted with the **Public Primary Key** using
  ...
  via `crypto_box_seal`.
- **API Challenges** and **API Access Tokens** are signed with the **Secret
...
- The **Public Signing Key** is derived from the **Private Signing Key** using
  ...
  `getPublicKey`.
- The **Secret Server Key** is 32 random bytes generated by a CSPRNG.
...
- **Secret Server Key**: A symmetric key generated by the server that is used
  to sign **API Challenges** and **API Access Tokens**.
...
- **Server Key ID**: A non-secret identifier for a **Secret Server Key** that
  is unique within the context of a **Form**.
- **Current Secret Server Key**: The current **Secret Server Key** in the key
  rotation.
- **Previous Secret Server Key**: The previous **Secret Server Key** in the key
  rotation.
